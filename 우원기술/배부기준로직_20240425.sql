        ...
        SELECT @CostYMFr = @CostYM + N'01', @CostYMTo = @CostYM + N'31'
         ...
         SELECT         F.AccUnit                 , a.PJTSeq,
                        D.CCtrSeq as recCCtrSeq               , ISNULL(D.PJTItemSeq, 0)    as     PJTItemSeq        , 
                        A.ManHour * (ISNULL(G.ConvNum,1)/ISNULL(G.ConvDen,1)) as manhour,     
                       ISNULL(A.HogiItemSeq, 0) as HogiItemSeq,b.MngDeptSeq,bb.cctrseq AS Hrcctrseq,dd.AllocSeq
         ...
         FROM  _TPJTResultHumanRes       AS A WITH(NOLOCK)
            ...
            --   AND B.IsCommon      = N'0' --공통비 아닌 것     
            --   AND B.IsNotUse      = N'0'  
            --   AND c.DriverSeq     = 30) EE ON EE.RevCCtrSeq = D.CCtrSeq 
    ...
    select reccctrseq,sum(manhour),allocseq from #temp
    ...
    drop table #temp
           ...
           --  select * from _TESMDCostKey	where costym= '202212'	
                  --select @CostYMFr,@CostYMTo

                  --select A.WorkStartDate,A.WorkEndDate,* 
                  --  from _TPJTResultHumanRes  as a 
                  -- WHERE A.WorkStartDate >= @CostYMFr
                  --   AND A.WorkEndDate   <= @CostYMTo


                  -- select  * from _tdacctr



                  --        SELECT A.CompanySeq   , A.CostUnit     ,A.CostKeySeq      ,A.AllocSeq      , B.SendCCtrSeq   ,
                  --     T.CostAccSeq   ,SUM(T.RevAmt + T.Amt)
                  --FROM  _TESMPDProdAllocName                AS A WITH (NOLOCK)
                  --      JOIN _TESMPDProdCCtrSend            AS B WITH (NOLOCK) ON A.CompanySeq  = B.CompanySeq 
